...
`동시성 문제`는 여러 프로세스나 스레드가 동시에 같은 데이터에 접근하려 할 때 데이터의 일관성을 해칠 수 있는 문제를 말합니다.

### 공유 자원(Shared Resource)
`공유 자원`은 여러 프로세스나 스레드가 접근할 수 있는 자원을 말합니다.

### 경쟁 상태(Race Condition)
`공유 자원`을 여러 프로세스나 스레드가 동시에 읽거나 쓰는 상황을 말하며,`공유 자원`에 접근하는 순서에 따라 실행 결과가 달라지는 상황 `경쟁 상태`라고 합니다.
